...
int APIENTRY wWinMain(HINSTANCE instance, HINSTANCE prev_instance, PWCHAR cmd_line, int cmd_show) {
	...
	if(RegisterClass(&window_class)) {
		...
		if(window) {
			...
			// Scene cube = {0};
			// cube.vertex_array = (Textured_Vertex[5]) {
			...
			void *scratch_buffer = smalloc(MEGABYTES(1));
			void *permanent_buffer = smalloc(MEGABYTES(1));
			
			Arena scratch_arena = arena_create(scratch_buffer, MEGABYTES(1));
			Arena permanent_arena = arena_create(permanent_buffer, MEGABYTES(1));
			
			int obj_file_size;
			byte *scene_data = read_file("../assets/cube.obj", &obj_file_size);
			Scene scene = obj_parse(scene_data, obj_file_size, &permanent_arena, &scratch_arena);
			free(scene_data);
